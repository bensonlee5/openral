"""deadman_watchdog_node SIGINT teardown contract — structural regression guard.
...
Before issue #290, ``deadman_watchdog_node.main`` wrapped ``rclpy.spin(node)``
...
``main()`` is exiting the watchdog has already fired its estop on
...
def _parse() -> ast.Module:
    """Parse deadman_watchdog_node as Python. Fail loudly if missing."""
    assert _NODE.is_file(), f"deadman_watchdog_node not found at {_NODE}"
...
def test_imports_external_shutdown_exception() -> None:
    ...
    assert "ExternalShutdownException" in imported, (
        "deadman_watchdog_node must import ExternalShutdownException from rclpy.executors. "
        f"Found imports from rclpy.executors: {sorted(imported)}"
    )


def test_no_bare_rclpy_shutdown_call() -> None:
    """``rclpy.shutdown()`` may not be called anywhere in deadman_watchdog_node.
    ...
    assert not bare_calls, (
        "deadman_watchdog_node must use rclpy.try_shutdown() (idempotent); "
...
def test_uses_try_shutdown() -> None:
    ...
    assert try_shutdown_lines, (
        "deadman_watchdog_node must call rclpy.try_shutdown() on the teardown path."
    )
...
def test_spin_wrapped_in_sigint_except() -> None:
    ...
    for node in ast.walk(tree):
        ...
        for stmt in node.body:
            ...
            break
    assert spin_trys, (
        "no `try:` block wraps a `.spin()` call in deadman_watchdog_node — the "
