...
- `ensure_act_onnx(repo_id, *, onnx_uri, cache_dir=None) -> Path` — resolve the ACT ONNX graph: an existing local file is used as-is; `hf://<repo>/<file>` is `hf_hub_download`ed (the shipped `model.onnx`). Missing `onnx_uri` is a hard `ROSConfigError` (export once with `tools/export_act_onnx.py`, publish, declare `policy_extras.act_onnx_uri`) — no silent local export. (L44)
...
- `maybe_attach_act_trt_from_env(policy, repo_id, *, onnx_uri, device="cuda:0") -> bool` — the single opt-in seam: attaches iff `OPENRAL_ACT_TRT` is truthy, else returns `False` so the caller keeps the torch path (`torch.compile`). (L193)
