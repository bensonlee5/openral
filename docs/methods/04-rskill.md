...
- `class SmolVLAAdapter(rSkillBase)` — Drives any SmolVLA-family policy. (L114)
  ...
  - `on_warmup() -> None` — Dummy inference. (L256)
  - `_configure_impl()` — Validate IO shapes match `n_dof`. (L289)
  - `_activate_impl()` — Reset policy, start `ChunkedExecutor`. (L305)
  - `_deactivate_impl()` — Stop pre-fetch, keep weights. (L313)
  - `_shutdown_impl()` — Stop threads, free GPU memory. (L319)
  - `_step_impl(world_state) -> Action` — One S1 step. (L336)
  - `_preprocess(raw) -> dict[str, Any]` — Lerobot preprocessor + tensor → device. (L372)
- `class SO100SmolVLASkill(SmolVLAAdapter)` — Pre-configured for the SO-100 6-DoF arm. (L428)
  - `__init__(prompt, *, repo_id='lerobot/smolvla_base', device='cuda:0', extra_images=None, **kwargs)` (L450)
- `_so100_obs_fn(world_state, *, device, extra_images=None, prompt) -> dict[str, Any]` — SO-100 WorldState → SmolVLA raw input. (L386)
...
- `ensure_act_onnx(repo_id, *, onnx_uri, cache_dir=None) -> Path` — resolve the ACT ONNX graph: an existing local file is used as-is; `hf://<repo>/<file>` is `hf_hub_download`ed (the shipped `model.onnx`). Missing `onnx_uri` is a hard `ROSConfigError` (export once with `tools/export_act_onnx.py`, publish, declare `policy_extras.act_onnx_uri`) — no silent local export. (L49)
...
- `maybe_attach_act_trt_from_env(policy, repo_id, *, onnx_uri, device="cuda:0") -> bool` — the single opt-in seam: attaches iff `OPENRAL_ACT_TRT` is truthy, else returns `False` so the caller keeps the torch path (`torch.compile`). (L200)
