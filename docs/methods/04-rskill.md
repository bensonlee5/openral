...
- `_find_repo_root_from(start) -> Path | None` — Walk up from `start` for the first ancestor containing both `pyproject.toml` and `rskills/`. (L957)
- `_validate_skill_ref(raw) -> str` — Validate and return a bare rSkill reference unchanged. Accepts bare names, `rskills/<name>` paths, or HF repo ids; rejects inputs carrying a known URI scheme (`hf://`, `local://`, `file://`, `http(s)://`). Private — used internally by the CLI and loader. (L970)
- `load_rskill_manifest(uri) -> RSkillManifest` — Resolve a bare skill reference to a parsed manifest. Tries local path → in-tree mapping → HF Hub download. In-process memoised. (L1010)
- `resolve_rskill_to_hf(uri) -> str` — Resolve a skill reference to either the underlying HF Hub repo id (`hf://...`) or an absolute local path (`local://...`); both forms are accepted by `from_pretrained` helpers. (L1082)
- `resolve_rskill_to_hf_with_revision(uri) -> tuple[str, str | None]` — Like `resolve_rskill_to_hf` but splits the optional `@<branch-or-sha>` pin off an `hf://` `weights_uri` into a separate `revision` so loaders can pass it to `from_pretrained`/`snapshot_download` instead of gluing it onto the repo id where HF drops it (security audit 2026-06, H4). (L1116)
...
- `class SmolVLAAdapter(rSkillBase)` — Drives any SmolVLA-family policy. (L113)
  - `__init__(repo_id, obs_fn, prompt, *, device='cuda:0', n_dof=6, n_cameras=None, prefetch_at=5, name='smolvla', version='0.1.0', embodiment_tags=None, latency_budget_ms=None)` — `n_cameras` (default `len(config.image_features)`) truncates warmup to the cameras the deploy feeds and threads to the TRT export (phantom-camera fix). (L158)
  - `on_load_weights() -> None` — Fetch checkpoint from HF Hub. (L196)
  - `on_warmup() -> None` — Dummy inference. (L246)
  - `_configure_impl()` — Validate IO shapes match `n_dof`. (L279)
  - `_activate_impl()` — Reset policy, start `ChunkedExecutor`. (L295)
  - `_deactivate_impl()` — Stop pre-fetch, keep weights. (L303)
  - `_shutdown_impl()` — Stop threads, free GPU memory. (L309)
  - `_step_impl(world_state) -> Action` — One S1 step. (L326)
  - `_preprocess(raw) -> dict[str, Any]` — Lerobot preprocessor + tensor → device. (L362)
- `class SO100SmolVLASkill(SmolVLAAdapter)` — Pre-configured for the SO-100 6-DoF arm. (L418)
  - `__init__(prompt, *, repo_id='lerobot/smolvla_base', device='cuda:0', extra_images=None, **kwargs)` (L440)
- `_so100_obs_fn(world_state, *, device, extra_images=None, prompt) -> dict[str, Any]` — SO-100 WorldState → SmolVLA raw input. (L376)
...
_TensorRT runtime for SmolVLA — swaps ``VLAFlowMatching.sample_actions`` for the split-ONNX TRT engines; everything around that seam (processor pipeline, prepare_images/state, queues, un-pad, postprocessor, ChunkedExecutor) keeps running upstream lerobot code. Opt-in via ``OPENRAL_SMOLVLA_TRT=1`` (+ ``OPENRAL_SMOLVLA_TRT_PRECISION``, default ``bf16``) through the shared ``maybe_attach_trt_from_env`` seam, wired into **both** SmolVLA load paths — the ``openral_rskill`` rSkill adapter and the ``openral_sim`` deploy/sim policy factory (``_build_smolvla``, where it replaces ``torch.compile``). Loud log, no silent fallback. Heavy deps deferred. Verified live in the real SO-101 deploy graph (engines load from cache in ~0.9 s, ``sample_actions`` swapped on the on-hardware policy)._

- `maybe_attach_trt_from_env(policy, repo_id, *, device="cuda:0", n_cameras=None) -> bool` — the single opt-in seam shared by both SmolVLA load paths: attaches the TRT runtime iff `OPENRAL_SMOLVLA_TRT` is truthy (reading `OPENRAL_SMOLVLA_TRT_PRECISION`, default `bf16`), else returns `False` so the caller keeps the torch path (`torch.compile`). Parses the CUDA ordinal from `device`. `n_cameras` (default `len(config.image_features)`) threads the deploy's *resolved* camera count to the export — the deploy factory passes `len(_camera_keys)` so a checkpoint that inherits unused `smolvla_base` camera slots (declares 3, `empty_cameras=0` drops the 3rd at inference) exports a 2-camera engine, not a 3-camera one that attends a phantom camera. (L64)
- `ensure_smolvla_onnx(repo_id, *, cache_dir=None, n_cameras=None) -> SmolVLAOnnxPaths` — cached split-ONNX export per checkpoint under `~/.cache/openral/smolvla_onnx/<repo>[-<n>cam]/` (first use ~7 min CPU on a separate fp32 copy — the deploy policy is untouched); cache hits recover `n_cameras`/tokens from the vision graph's static shapes. `n_cameras` (default `len(config.image_features)`) is baked into the graphs and keyed into the cache slug so 2- and 3-camera engines for the same repo never collide; pass the deploy's true camera count for checkpoints that over-declare camera slots. (L108)
- `attach_trt_sample_actions(policy, repo_id, *, precision="bf16", device_index=0, cache_dir=None, n_cameras=None) -> None` — builds/loads the two `TensorRTRuntime` engines (EngineCache-keyed `<repo>#vision` / `<repo>#policy`) and replaces `policy.model.sample_actions` with the TRT-backed callable (original kept at `policy.model._openral_torch_sample_actions`). `fp16` is rejected with the measured rationale (~60% action error — flow matching amplifies fp16 layernorm overflow); `bf16` measured at 1.6% vs fp32 on a real training-set sample (PR #139). The callable enforces the graph's static contract: all cameras present, all img_masks true, lang padded to `tokenizer_max_length`; RTC kwargs rejected; noise sampled host-side when `None`. ADR-0082 Phase 3: `set_precomputed_img_embs(embs)` stashes `(n_cameras, T, hidden)` embeddings from the zero-copy NVMM vision leg (`NvmmVisionEncoder` on the *same* cached vision engine); the next `__call__` then skips its own image round-trip + vision inference — one-shot, cleared on use so a stale frame never leaks into a later chunk. `n_cameras` / `vision_onnx` / `vision_rskill_id` / `quantization` / `device_index` [@property] expose the engine-sharing facts the encoder needs to cache-hit the identical engine. (L363)

### `python/rskill/src/openral_rskill/act_trt.py`
_ONNX/TensorRT runtime for ACT — the ACT analogue of `smolvla_trt`. ACT is a plain CNN+transformer, so the *whole* model exports to one ONNX graph (`tools/export_act_onnx.py`); this module attaches it by swapping `ACTPolicy.predict_action_chunk` (the one method `select_action` calls — queue, chunk replay, and external MEAN_STD postprocessor all keep running upstream lerobot code). Runs through the generic `Runtime` backends: `TensorRTRuntime` when `tensorrt` is importable on CUDA (engine built + cached on first load), else `ONNXRuntime` (CUDA/CPU). Opt-in via `OPENRAL_ACT_TRT=1`, wired into `_build_act` where it replaces `torch.compile`. Loud log, no silent fallback. Heavy deps deferred. Verified CPU-parity (max abs diff 7.7e-07 vs torch `predict_action_chunk` on the real gabrycina/so101-passing-pen checkpoint)._

- `ensure_act_onnx(repo_id, *, onnx_uri, cache_dir=None) -> Path` — resolve the ACT ONNX graph: an existing local file is used as-is; `hf://<repo>/<file>` is `hf_hub_download`ed (the shipped `model.onnx`). Missing `onnx_uri` is a hard `ROSConfigError` (export once with `tools/export_act_onnx.py`, publish, declare `policy_extras.act_onnx_uri`) — no silent local export. (L48)
- `attach_act_onnx_runtime(policy, repo_id, *, onnx_uri, device="cuda:0", cache_dir=None) -> None` — loads the graph into a `TensorRTRuntime` (CUDA + `tensorrt`) or `ONNXRuntime`, then swaps `policy.predict_action_chunk` for the `_OnnxActChunk` runner (original kept at `policy._openral_torch_predict_action_chunk`). The runner feeds `{state, img_<cam>…}` (camera order from `config.image_features`) and returns the `(B, chunk, action_dim)` normalized chunk as a torch tensor on `device`. (L156)
- `maybe_attach_act_trt_from_env(policy, repo_id, *, onnx_uri, device="cuda:0") -> bool` — the single opt-in seam: attaches iff `OPENRAL_ACT_TRT` is truthy, else returns `False` so the caller keeps the torch path (`torch.compile`). (L197)

### `python/rskill/src/openral_rskill/_vla_core.py`
_Shared helpers for VLA adapters (Layer 3); internal — no public re-export._

- `InferenceKind` — `Literal["foreground", "prefetch", "single"]` for the `inference.kind` span attribute. (L39)
- `resolve_device(spec: VLASpec) -> str` — `"auto"` → `"cuda:0"` / `"mps"` / `"cpu"` against real torch. (L42)
- `resolve_rskill_repo_id(weights_uri: str, *, adapter_name: str) -> str` — Validate skill reference and resolve to bare HF repo id; `adapter_name` is used in the `ROSConfigError` message. (L67)
- `resolve_rskill_repo_revision(weights_uri: str, *, adapter_name: str) -> tuple[str, str | None]` — Like `resolve_rskill_repo_id` but also returns the optional `@<sha>` revision pin (threaded into `from_pretrained`/`snapshot_download` by the sim adapters) and warns `rskill.unpinned_weights` when an `hf://` skill is unpinned (security audit 2026-06, H4). (L98)
- `apply_chunk_replay(policy, spec_extra) -> int` — Override `policy.config.n_action_steps` from `vla.extra` (default `chunk_size // 2`); used by all lerobot-style adapters (`smolvla`, `act`, `pi05`) to amortise the heavy chunk forward over multiple env steps. (L274)
- `_CUDAGRAPH_COMPILE_MODES` — `frozenset({"reduce-overhead", "max-autotune"})`; the `torch.compile` modes that may capture CUDA graphs and therefore require output cloning (static replay buffers would otherwise be overwritten under lerobot's queued action views / `ChunkedExecutor` pre-fetch). (L333)
- `_has_bnb_quantized_modules(policy) -> bool` — True when any submodule's class comes from `bitsandbytes` (`Linear4bit` / `Linear8bitLt` rewrites from `openral_sim._quantization`). Class-module-path check; never imports bnb. (L345)
- `_clone_chunk_output(out, torch) -> Any` — Recursively `.clone()` every tensor in a chunk forward's output (tensor / tuple / list / dict; non-tensor leaves pass through) so downstream holders own their storage, detached from CUDA-graph static buffers. (L359)
- `maybe_compile_chunk_forward(policy, spec_extra, device, torch, *, method_name="_get_action_chunk") -> bool` — Best-effort `torch.compile` of the chunk forward with a runtime fallback wrapper (latches into eager mode on backend errors). Skipped on CPU and when `vla.extra.compile` is falsy. Two safety gates: bitsandbytes-quantized policies are never compiled (mixed nf4/bf16 graphs trip dtype-mismatch errors — the same reason the pi05 adapter forces `compile_model = False`; logs `vla_compile_skipped_bnb_quantized`), and under cudagraph modes (`_CUDAGRAPH_COMPILE_MODES`) every output is routed through `_clone_chunk_output` on both the compiled and eager-fallback branches. Logs `vla_compile_setup_failed` / `vla_compile_runtime_fallback` on failure. (L377)
...
- `to_numpy_action(action_tensor) -> NDArray[np.float32]` — `(1, A)` torch tensor → 1-D float32 NumPy. (L536)
- `parse_hf_file_uri(uri: str) -> tuple[str, str | None, str]` — Splits `hf://owner/repo[@rev]/path/to/file.ext` into `(repo_id, revision, filename)` for per-file `hf_hub_download` calls. Rejects bare-repo URIs with a typed `ROSConfigError`. (L615)
- `materialize_processor_dir(manifest: RSkillManifest) -> str` — Downloads the manifest's per-file `processors` artefacts (Gap 1+3 of the rSkill self-containment audit) via `hf_hub_download` calls and symlinks them under the lerobot-canonical filenames (`policy_preprocessor.json` / `policy_postprocessor.json`) in a fresh temp directory. Also walks each downloaded JSON's `steps[*]` and downloads any sibling `state_file` (normalizer / unnormalizer `.safetensors`) into the same staging dir so lerobot's `PolicyProcessorPipeline.from_pretrained(<dir>)` resolves every step locally without falling back to `hf_hub_download(repo_id=<dir>)`. Single seam used by the SmolVLA and modern-ACT adapters; raises `ROSConfigError` if `manifest.processors is None`. Every download is routed through `_hf_download_cached_first` so a cache-hit avoids the per-file HF HEAD validation that otherwise stacks 3–5 seconds onto every load. (L665)
