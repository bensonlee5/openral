...
- `class SmolVLAAdapter(rSkillBase)` — Drives any SmolVLA-family policy. (L113)
  ...
  - `on_warmup() -> None` — Dummy inference. (L234)
  - `_configure_impl()` — Validate IO shapes match `n_dof`. (L262)
  - `_activate_impl()` — Reset policy, start `ChunkedExecutor`. (L278)
  - `_deactivate_impl()` — Stop pre-fetch, keep weights. (L286)
  - `_shutdown_impl()` — Stop threads, free GPU memory. (L292)
  - `_step_impl(world_state) -> Action` — One S1 step. (L309)
  - `_preprocess(raw) -> dict[str, Any]` — Lerobot preprocessor + tensor → device. (L345)
- `class SO100SmolVLASkill(SmolVLAAdapter)` — Pre-configured for the SO-100 6-DoF arm. (L401)
  - `__init__(prompt, *, repo_id='lerobot/smolvla_base', device='cuda:0', extra_images=None, **kwargs)` (L423)
- `_so100_obs_fn(world_state, *, device, extra_images=None, prompt) -> dict[str, Any]` — SO-100 WorldState → SmolVLA raw input. (L359)
