...
_Temporary fallback boot helper + stateless scoring server for the Robometer-4B reward monitor (ADR-0057), companion to `openral_runner.backends.reward.robometer_reward.RobometerReward`._ The default runtime is now `RobometerInProcessReward`, which imports `_robometer_server.py::_Scorer` directly inside `reward_monitor_node`; the ZMQ sidecar remains only for `OPENRAL_ROBOMETER_BACKEND=sidecar` until deploy-sim validates the in-process path. As of lerobot 0.6.0 the reward model is lerobot's in-tree `lerobot.rewards.robometer.RobometerRewardModel` — a vanilla `AutoModelForImageTextToText` (Qwen3-VL-4B) loaded with plain `transformers`. There is **no** pinned `robometer` git package, **no** `transformers==4.57.1` force-pin, and **no** dedicated venv. The scorer keeps OpenRAL's NF4 pre-quantized checkpoint (`OpenRAL/rskill-robometer-4b-nf4`, ~3.3 GB resident), meta-builds the native `RobometerRewardModel` skeleton and drops the packed 4-bit weights (remapped into the native module) in directly — no bf16 spike, no Qwen weight download. The fallback sidecar answers a ZMQ REQ/REP + msgpack protocol (`{"op":"score","frames","n","width","height","task","num_bins"}` → `{"ok","progress","success"}`). Validated live: 3.33 GB NF4, progress ramps to 0.88 + success 0.90 at task completion.
