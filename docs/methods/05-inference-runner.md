...
- `_build_event_tee_branch(spec, platform) -> str` (L649) — ADR-0018 F6 — Returns the event leg of the `tee`: lifts NVMM to system memory, pins `format=BGR`, rate-caps via `videorate` to `event_rate_hz`, terminates in `appsink name=event_sink`.
- `_build_ros_tee_branch(spec, platform) -> str` (L633) — Returns the observability leg (system memory BGR `appsink name=ros_sink`).
- `_lift_convert(platform) -> str` (L674) — The converter a tee leg uses to lift NVMM → system-memory BGR: `nvvidconv` (Tegra) / `nvvideoconvert` (DeepStream) / `videoconvert` (else). ADR-0082.
...
_ctypes binding to `libnvbufsurface.so` (NVMM→CUDA zero-copy): `load()` (memoised probe), `wrap_buffer(addr) -> NvBufSurfaceHandle` (typed `gpu_ptr/width/height/pitch/color_format/size` view of a mapped NVMM `Gst.Buffer`), struct mirrors of NVIDIA's `nvbufsurface.h`._

- `class StableSurfaceMirror` (L316) — ADR-0082 Phase 3. Reader-owned GPU **double buffer** decoupling a latched handle from the Gst buffer pool: `mirror(handle) -> NvBufSurfaceHandle` DtoD-copies the surface into one of two `cudaMalloc` buffers (alternating) while the source map is provably valid, so the Gst buffer unmaps immediately and an async consumer racing the next frame sees at worst a *torn frame*, never a use-after-free. Requires cuda-python (raises `NvBufSurfaceLibraryError` otherwise). `close()` frees the buffers; idempotent. Used by `GStreamerSensorReader._handle_nvmm_buffer` — every latched NVMM handle now points at mirror-owned memory.

### `python/runner/src/openral_runner/backends/gstreamer/trt_nvmm.py`
_Clean-room zero-copy TensorRT executor (ADR-0037 PR5b) — runs a TRT engine directly on a CUDA device pointer (`NvBufSurface.dataPtr`) with no GPU->CPU copy. An **nvrtc**-compiled CUDA kernel (built to a SASS CUBIN for the local `sm_<cc>` — no PTX JIT; `rgba_to_nchw_norm` for the detector `/255` convention, or `rgba_resize_pad_norm` for the SmolVLA resize-with-pad `[-1,1]` convention, selected by the `preprocess` ctor mode) converts the pitch-padded RGBA frame to planar float32 NCHW straight into the engine input buffer; engine + kernel run on the device's CUDA **primary context** (made current by `cudaSetDevice`; `cuInit` initializes the driver API). Deserializes engine bytes from `TensorRTRuntime.serialized_engine`. Imports **cuda-python** (`cuda.bindings` driver/runtime/nvrtc) + tensorrt lazily at construction — **no pycuda, no shared context** — so the NVMM tier deploys in the lean ds-on image (no nvcc/g++); requires the `tensorrt` group + `nvrtc`._

- `resize_pad_geometry(src_h, src_w, dst_h, dst_w) -> tuple[int, int, int, int]` — `(resized_h, resized_w, pad_h, pad_w)` for lerobot-style aspect-preserving resize-with-pad (ratio = max of the two axes, `int()` truncation, top/left pad). Pure host math for the `resize_pad_pm1` kernel; unit-tested hardware-free against lerobot's `resize_with_pad`.
- `class TrtNvmmExecutor` (L156) — `__init__(engine_bytes, *, input_size=(h,w), device_index=0, preprocess="scale")` — API unchanged from the prior pycuda impl plus the `preprocess` mode: `"scale"` (default) keeps the RGBA→NCHW `/255` kernel with the frame-size==network-size contract; `"resize_pad_pm1"` compiles the `rgba_resize_pad_norm` kernel instead — aspect-preserving bilinear resize (align_corners=False, border-replicate edges) + top-left pad + `[-1,1]` normalization, the exact lerobot SmolVLA `resize_with_pad` → SigLIP convention, accepting any frame size (feeds VLA vision-encoder engines straight from an NVMM device pointer; GPU pixel parity in `tests/unit/test_trt_nvmm_executor.py::test_executor_resize_pad_pm1_matches_torch`). Internals reworked to nvrtc + cuda-python. Selects the device via `cudaSetDevice(device_index)` (initializes + makes the primary context current), nvrtc-compiles the kernel for the device's `sm_<cc>` to a SASS CUBIN (`nvrtcGetCUBIN`, no PTX JIT) and loads it via `cuModuleLoadData`/`cuModuleGetFunction`, deserializes the engine (`trt.Runtime(...).deserialize_cuda_engine`), creates the execution context, resolves the input batch/shape (see the batch note below), allocates the input + per-output device buffers (`cudaMalloc`) via `set_tensor_address`, and creates a cudart stream — with a `BaseException` guard (`_free_resources`) that frees partially-allocated buffers / unloads the module / destroys the stream before re-raising (a failed `__init__` returns no instance to `close()`); raises `ROSConfigError` if cuda-python/trt missing, engine deserialization fails, or the engine lacks exactly one input, `ROSRuntimeError` on a CUDA/nvrtc setup failure. **Batch (ADR-0082 Phase 2):** the engine input's *static* leading dim sets the slot count (`batch` [@property]; e.g. the SmolVLA vision graph's `(n_cameras,3,H,W)`), a dynamic dim keeps the original single-frame contract (`set_input_shape (1,3,h,w)`); the input dtype must be float32 (the kernels write floats) — else `ROSConfigError`. `infer_rgba_devptrs(frames) -> dict[str, np.ndarray]` — one `(src_ptr, width, height, pitch)` per batch slot in engine slot order; each frame's kernel launch writes its own slot of the input buffer (`_enqueue_preprocess`), then one `execute_async_v3`, outputs copied dtoh, stream synced; raises `ROSConfigError` on a frame-count mismatch or size-rule violation. `infer_rgba_devptr(src_ptr, *, width, height, pitch) -> dict[str, np.ndarray]` — single-slot convenience (requires batch 1); same semantics. `output_shapes() -> list[tuple[str, tuple[int, ...]]]` — `(name, shape)` per engine output, for output identification. `close()` — frees device buffers, unloads the kernel module, destroys the stream (shared best-effort `_free_resources` helper; non-zero CUDA teardown returns are logged, not raised); idempotent.
...
- `class NvmmObjectsDetector` (L36) — `__init__(onnx_path, *, labels, model_id, input_size=(640,640), score_threshold=0.5, device_index=0, quantization=None)` — validates labels non-empty, score_threshold in [0,1], ONNX path exists; builds the TRT engine via `TensorRTRuntime(device="cuda:<N>", rskill_id=model_id).serialized_engine(path)`, constructs `TrtNvmmExecutor`, identifies logits/boxes names via `identify_rtdetr_outputs(executor.output_shapes())` (closing the executor if identification raises, since a partial `__init__` returns no instance to `close()`), and records engine/executor/init timings in `last_timings_ms`; raises `ROSConfigError` on bad args or missing ONNX. `last_timings_ms -> dict[str,float]` [@property] returns the most recent init/detect timing breakdown. `detect_nvmm(handle, sensor_id) -> ObjectsMetadata | None` (L104) — calls `TrtNvmmExecutor.infer_rgba_devptr` with the handle's `gpu_ptr`/`width`/`height`/`pitch`, then delegates to `postprocess_rtdetr`; records `trt_infer_ms`, `postprocess_ms`, and `detect_total_ms`; returns `None` when no detection passes threshold. `close()` (L43) — delegates to `TrtNvmmExecutor.close()`; idempotent.

### `python/runner/src/openral_runner/backends/gstreamer/nvmm_vision_encoder.py`
_Zero-copy NVMM VLA vision encoder (ADR-0082 Phase 2) — runs the SmolVLA split-export `vision_encoder.onnx` as a TRT engine directly on NVMM camera frames: one `TrtNvmmExecutor` in `resize_pad_pm1` mode preprocesses each camera's RGBA device pointer into its batch slot (on-GPU lerobot `resize_with_pad` + SigLIP `[-1,1]`), emitting `(n_cameras, T_img, hidden)` image embeddings. No decoded pixel touches host memory; the device-side embedding handoff to the policy is Phase 3. Mirrors `NvmmObjectsDetector`; requires the `tensorrt` group + `nvrtc` (ds-on deployable, no pycuda)._

- `class NvmmVisionEncoder` (L42) — `__init__(onnx_path, *, model_id, device_index=0, quantization=None)` — builds (or cache-loads) the vision engine via `TensorRTRuntime.serialized_engine`, reads the network size from the ONNX input, constructs the executor in `resize_pad_pm1` mode; raises `ROSConfigError` on missing ONNX / non-static input. `n_cameras -> int` [@property] — the engine's camera slot count. `last_timings_ms -> dict[str,float]` [@property]. `encode_nvmm(handles) -> NDArray[np.float32]` (L42) — one `NvBufSurfaceHandle` per camera in checkpoint camera order → `(n_cameras, T_img, hidden)` float32 embeddings via `TrtNvmmExecutor.infer_rgba_devptrs`. `close()` — idempotent. Phase 2 acceptance gate: devptr embeddings vs the deployed host-numpy path on the *same cached bf16 engine* — measured max-rel 1.1%, per-token cosine > 0.999 (`tests/unit/test_nvmm_vision_encoder.py`).
...
- `class DeployRunner(InferenceRunnerBase)` — First end-to-end closer of the `WorldState → Skill → safety → HAL` loop on real hardware / digital twins. The runner is the safety-supervisor boundary per CLAUDE.md §10: catches `ROSSafetyViolation` from the SafetyClient, records it on the `TickResult`, withholds the `HAL.send_action` call (does not re-raise because withholding IS the mitigation today). (L77)
  - `__init__(*, hal, skill, aggregator, sensor_readers=(), safety_client=None, recorder=None, **base_kwargs)` — Caller must pre-`configure()`+`activate()` the skill; runner manages HAL + reader open/close. Defaults `safety_client` to `NullSafetyClient`. Dashboard JPEG thumbnails are emitted at a private fixed cadence. ADR-0019 PR3: optional `recorder` is a `openral_dataset.RolloutRecorder`; when set, `episode_start` / `episode_end` drive its lifecycle and every tick fans out via `record_frame`. (L124)
  - `episode_start(task_string: str) -> int` — ADR-0019 PR3: open a new episode on the attached recorder; returns the new `episode_idx` (or `-1` when no recorder is attached). Raises `RuntimeError` if called twice without `episode_end`. (L183)
  - `episode_end(*, success: bool) -> None` — ADR-0019 PR3: close the current recorder episode with the success flag. No-op when no recorder is attached. Raises `RuntimeError` if called without `episode_start`. (L211)
