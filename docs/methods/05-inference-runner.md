...
- `class RobometerInProcessReward` — `__init__(*, model_id, weights_source="OpenRAL/rskill-robometer-4b-nf4", num_bins=100, success_threshold=0.5, max_frames=8, device="cuda", fallback=None)` — default Robometer backend for `reward_monitor_node`: lazily imports `tools/_robometer_server.py::_Scorer`, meta-loads the prequantized NF4 checkpoint in the reward-monitor process, and scores BGR frames via the same native lerobot 0.6.0 `_compute_rbm_logits` + `decode_progress_outputs` path. `score(frames, task) -> (progress, success)` validates empty task/clip + frame sizes, evenly subsamples to `max_frames`, converts BGR→RGB, returns per-frame normalized arrays. `assess(...) -> dict` mirrors the reasoner contract. If the in-process loader is unavailable and `OPENRAL_ROBOMETER_INPROCESS_FALLBACK` is not false, it falls back to `RobometerReward` as a temporary deploy-sim bridge.
- `class RobometerReward` — `__init__(*, model_id, weights_source="robometer/Robometer-4B", host="127.0.0.1", port=5769, auto_spawn=True, boot_timeout_s=1200.0, request_timeout_s=180.0, num_bins=100, success_threshold=0.5)` — legacy ZMQ client + auto-managed lifecycle for the stateless reward sidecar (mirrors `QwenSceneVlm`). Kept only as a temporary fallback / `OPENRAL_ROBOMETER_BACKEND=sidecar` opt-in until deploy-sim validates the in-process Robometer path. `score(frames, task)` RPCs `{"op":"score",...}`; `close()` tears down the sidecar process group.
- `build_reward_monitor(manifest, *, host="127.0.0.1", port=5769) -> RobometerInProcessReward | RobometerReward | TOPRewardMonitor` — build from a `kind:"reward"` manifest; dispatches `reward.backend=="topreward"` to `TOPRewardMonitor`, defaults Robometer to `RobometerInProcessReward`, and honours `OPENRAL_ROBOMETER_BACKEND=sidecar` for the temporary ZMQ fallback. Raises `ROSConfigError` if `manifest.kind != "reward"`. Lazy.
